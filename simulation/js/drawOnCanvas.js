...
image.src = "./images/lines2.png"; // Replace with your actual image path
function goToNextPage(ele) {
  screen = 1;
  const screens = document.querySelectorAll(".simsubscreen");
  screens[0].classList.remove("grid");
...
function drawRectangle(a, color = "#00ff00") {
  ...
  ctx.fillStyle = color;
  ctx.beginPath();
  ctx.rect(a[0] * scaleX, a[1] * scaleY, 25, 25);
  ctx.stroke();
  ctx.fill();
  ctx.closePath();
}
...
function drawArc(a, color = "#00ff00") {
  ...
  ctx.strokeStyle = "white";
  ctx.fillStyle = color;
  ctx.ellipse(a[0] * scaleX, a[1] * scaleY, 9, 13, 0, 0, 4 * Math.PI);
  ctx.fill();
  ctx.stroke();
  ctx.closePath();
}
...
function resizeCanvas() {
  ...
  const container = canvasImg.parentElement;
  const rect = container.getBoundingClientRect();
  canvasImg.width = rect.width;
  canvasImg.height = rect.height;
...
function drawImage() {
  ...
  ctxImg.drawImage(image, 0, 0, canvasImg.width, canvasImg.height);
  // setUpCanvas();
  // drawOnCanvas1();
  // setUpCanvas();
}
...
function processFault1() {
  ...
  if (fault == "transient") {
    ...
    window.setTimeout(function () {
      ...
      timerTracker = window.setInterval(function () {
        setTimer1();
      }, 1000);
    }, 1500);
...
function processFault2() {
  ...
  document.getElementById(
    "controlContent"
  ).innerHTML = `At the instant of fault created, the fault is fed from the source. Relay and Recloser picks up during this instant. The line till the fault point gets loaded with fault current and the prefault load current.`;
  setUpCanvas();
  window.setTimeout(function () {
    document.getElementById(
      "controlContent"
    ).innerHTML = `Definite time delay timer at the recloser starts counting till the set value.`;
    timerTracker = window.setInterval(function () {
      setF2Timer1();
    }, 1000);
  }, 1500);
...
//function calls
let myInt1 = setInterval(function () {
  animateArrow();
}, 800);
let myInt2 = setInterval(function () {
  downArrowAnimate();
}, 1200);
